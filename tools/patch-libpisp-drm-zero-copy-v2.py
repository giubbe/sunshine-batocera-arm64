...
SRC = Path('src/examples/convert.cpp')
s = SRC.read_text()
...
MESON = Path('src/examples/meson.build')
m = MESON.read_text()
old = "opts_dep = dependency('cxxopts', fallback : ['cxxopts', 'cxxopts_dep'])\n"
if m.count(old) != 1:
    raise SystemExit('unexpected cxxopts stanza')
m = m.replace(old, old + "drm_dep = dependency('libdrm')\n", 1)
old = 'dependencies: [libpisp_dep, opts_dep],'
if m.count(old) != 1:
    raise SystemExit('unexpected dependency list')
m = m.replace(old, 'dependencies: [libpisp_dep, opts_dep, drm_dep],', 1)
MESON.write_text(m)
